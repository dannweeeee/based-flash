...
export default function BlockInfo({
  ...
  const progressPercentage = Math.min((timeSinceLastBlock / refreshInterval) * 100, 100);
  const isFlashblock = refreshInterval < 1000;
  
  return (
    <div className={`rounded-lg border ${isFlashblock ? 'border-blue-500 dark:border-blue-700' : 'border-gray-200 dark:border-gray-800'} p-6 ${className} relative overflow-hidden`}>
      ...
      <div className="relative z-10">
        <div className="flex justify-between items-center mb-4">
          <div>
            <h2 className={`text-xl font-bold ${isFlashblock ? 'text-blue-600 dark:text-blue-400' : ''}`}>{title}</h2>
            <div className="text-xs text-gray-500 mt-1">
              {blockCount > 0 && `${blockCount} blocks observed`}
            </div>
          </div>
          <div className={`text-sm ${isFlashblock ? 'text-blue-600 dark:text-blue-400 font-semibold' : 'text-gray-500'}`}>
        ...
        {isLoading && !blockData ? (
          <div className="flex justify-center items-center h-40">
            <div className={`animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 ${isFlashblock ? 'border-blue-500' : 'border-gray-500'}`}></div>
          </div>
        ) : error ? (
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="p-3 bg-gray-50 dark:bg-gray-800 rounded-lg">
                ...
                <div className="font-medium">{Number(blockData.gasUsed).toLocaleString()}</div>
              </div>
            ...
            <div className="mt-4">
              <div className="flex justify-between text-sm mb-1">
                ...
                <span className={`text-sm font-medium ${isFlashblock ? 'text-blue-600 dark:text-blue-400' : ''}`}>
                  {Math.max(0, Math.floor((refreshInterval - timeSinceLastBlock) / 100) / 10).toFixed(1)}s
                </span>
              </div>
              <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2.5 overflow-hidden">
                <div 
                  className={`${isFlashblock ? 'bg-blue-600' : 'bg-gray-600'} h-2.5 rounded-full transition-all duration-100 ease-linear`}
